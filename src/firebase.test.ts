...
describe('Firebase save migrations',()=>{
  it('preserves economy, inventory, and identity while adding quest metadata',()=>{const old={...initialState(),player:'Returning Wanderer',dewdrops:777,saveRevision:undefined,recentActions:undefined,materials:undefined,cosmetics:undefined,questRecords:undefined,questHistory:undefined} as unknown as ReturnType<typeof initialState>;const migrated=migrateGameState(old);expect(migrated.player).toBe('Returning Wanderer');expect(migrated.dewdrops).toBe(777);expect(migrated.saveRevision).toBe(0);expect(migrated.recentActions).toEqual([]);expect(migrated.materials).toEqual({});expect(Object.keys(migrated.questRecords).length).toBeGreaterThan(0);expect(CURRENT_SCHEMA_VERSION).toBe(9)});
