...
export interface EncounterHistoryEntry {id:string;species:WildSpecies;result:'success'|'miss';region:string;at:number}
export interface GardenPlot {id:number;seedId?:string;plantedAt?:number;wateredAt?:number;assisted?:boolean}
export interface GameState {saveRevision:number;recentActions:string[];player:string;creatureName:string;species:Species;needs:Record<NeedKey,number>;dewdrops:number;starpetals:number;inventory:Item[];materials:Record<string,number>;discoveredMaterials:string[];discoveredRecipes:string[];cosmetics:string[];questRecords:Record<string,QuestRecord>;questHistory:string[];encounterHistory:EncounterHistoryEntry[];claimedAchievementTiers:string[];equipped:string[];roomTheme:string;dailyCare:number;dailyCareDate:string;claimed:boolean;achievements:string[];lastAction:string;creatures:OwnedCreature[];activeCreatureId:string;discoveries:WildSpecies[];encounterProgress:Partial<Record<WildSpecies,number>>;encounterDays:Partial<Record<WildSpecies,string[]>>;encounterRotationDate:string;encounterRotation:WildSpecies[];encounterAttemptDate:string;dailyEncounterAttempts:Partial<Record<WildSpecies,number>>;fieldDays:string[];forageDate:string;foragePlays:number;glassrootStoryStep:number;glassrootGatherDate:string;glassrootGatherPlays:number;glassrootWeatherDate:string;glassrootWeather:'clear'|'mist'|'prism-squall';gardenPlots:GardenPlot[];seeds:Record<string,number>;discoveredSeeds:string[];gardenWeatherDate:string;gardenWeather:'mild'|'rain'|'sun';gardenAssistDate:string}
