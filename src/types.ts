...
export interface OwnedCreature {id:string;species:CreatureSpecies;name:string;rarity:CreatureRarity;bond:number;origin:string}
...
export interface GameState {player:string;creatureName:string;species:Species;needs:Record<NeedKey,number>;dewdrops:number;starpetals:number;inventory:Item[];equipped:string[];roomTheme:string;dailyCare:number;claimed:boolean;achievements:string[];lastAction:string;creatures:OwnedCreature[];activeCreatureId:string;discoveries:WildSpecies[];encounterProgress:Partial<Record<WildSpecies,number>>}
